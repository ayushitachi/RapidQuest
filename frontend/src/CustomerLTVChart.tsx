...
const CustomerLTVChart = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_PATH}/api/customer-ltv`
      );
      const ltvData = await response.json();

      // Ensure data formatting is consistent with the new API response
      const formattedData = ltvData.map(
        (item: { month: any; averageRevenue: number; customerCount: any }) => ({
  ...
  const formatYAxis = (tickItem: { toLocaleString: () => any }) => {
