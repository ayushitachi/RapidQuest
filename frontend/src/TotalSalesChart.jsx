...
const TotalSalesChart = () => {
    ...
    const fetchData = async (selectedTimeFrame) => {
        ...
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_PATH}/api/total-sales?timeFrame=${selectedTimeFrame}`);
            if (!response.ok) {
    ...
    const formatDate = (dateObj, selectedTimeFrame) => {
    ...
    return (
        <div>
            ...
            {isLoading ? (
                ...
                <div style={{ width: '100%', height: 300, overflowX: timeFrame === 'daily' ? 'scroll' : 'hidden' }}>
                    <div style={{ width: timeFrame === 'daily' ? `${data.length * 50}px` : '100%', minWidth: '100%' }}>
